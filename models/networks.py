...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    if netF == 'global_pool':
        ...
        if opt.usehmgmodification and opt.K_num_patches>0: #hmg修改
            net = PatchSampleF_K(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
        else:
...
class GANLoss2(nn.Module): #hmg添加
    """Define different GAN objectives.
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0,ganloss_beta=0.2):
        """ Initialize the GANLoss class.
        ...
        if gan_mode == 'lsgan':
            ...
            raise NotImplementedError('gan mode %s not implemented' % gan_mode)
        self.ganloss_beta=ganloss_beta
        self.ganloss_beta_1=1-ganloss_beta

    def get_target_tensor(self, prediction, target_is_real,realimpre):
        """Create label tensors with the same size as the input.
        ...
        if target_is_real:
            ...
            target_tensor = self.fake_label
        target_tensor=target_tensor.expand_as(prediction) #realimpre.clip(-1,1) 截断
        if realimpre is not None:
            if target_is_real: target_tensor=target_tensor*self.ganloss_beta_1+self.ganloss_beta*realimpre
            else: target_tensor=target_tensor+self.ganloss_beta*(1-realimpre)
            
        return target_tensor

    def __call__(self, prediction, target_is_real,realimpre=None):
        """Calculate loss given Discriminator's output and grount truth labels.

        Parameters:
            prediction (tensor) - - tpyically the prediction output from a discriminator
            target_is_real (bool) - - if the ground truth label is for real images or fake images
	    realimpre (tensor) - - real image prediction
        ...
        bs = prediction.size(0)
        if self.gan_mode in ['lsgan', 'vanilla']:
            target_tensor = self.get_target_tensor(prediction, target_is_real,realimpre)#hmg注意，错了
            loss = self.loss(prediction, target_tensor)
        elif self.gan_mode == 'wgangp':
            if target_is_real:
                loss = -prediction.mean()
            ...
            if target_is_real:
                ...
                loss = F.softplus(prediction).view(bs, -1).mean(dim=1)
        elif self.gan_mode == 'hinge': #max(1-p*p^,0)=-min(p*p^-1,0);
            if realimpre is not None:  #(beta_*p+beta*r)* p^=p*  (beta_+beta*r/p)*p^
	    	## realimpre=torch.clamp(realimpre,min,max) # 把值截断在min到max之间
                # realimpreview=realimpre.view(realimpre.shape[0],-1)
                # realimpreview/=abs(realimpreview).max(dim=1)[0].unsqueeze(dim=1)
                # if target_is_real:prediction = (1+self.ganloss_beta*realimpre)*prediction  #p*p^
                # else:prediction = (1-self.ganloss_beta*(-realimpre))*prediction  #p*p^
                if target_is_real:prediction = (self.ganloss_beta_1+self.ganloss_beta*realimpre)*prediction  #p*p^
                else:prediction = (self.ganloss_beta_1-self.ganloss_beta*(-realimpre))*prediction  #p*p^
                
            if target_is_real:
                minvalue = torch.min(prediction - 1, torch.zeros(prediction.shape).to(prediction.device)) #prediction=1*p^
                loss = -torch.mean(minvalue)
            else:
                minvalue = torch.min(-prediction - 1, torch.zeros(prediction.shape).to(prediction.device))#-prediction=-1*p^
                loss = -torch.mean(minvalue)
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                ...
                x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1]) # B,256,128->B*256,128
            else:
...
class PatchSampleF_K(nn.Module): #hmg添加，能使用这个函数说明已经满足了opt.usehmgmodification and opt.K_num_patches，因此本函数里不再重复
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleF_K, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None,K_num_patches=1):
        return_ids = []
        return_feats = []
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
        for feat_id, feat in enumerate(feats):
            ...
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)
            if num_patches > 0:
                if patch_ids is not None: #q
                    patch_id = patch_ids[feat_id]
                    x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1]) # B,256,128->B*256,128
                else: #k
                    patch_id = torch.randperm(feat_reshape.shape[1], device=feats[0].device)
                    patch_id = patch_id[:int(min(num_patches*K_num_patches, patch_id.shape[0]))]  # .to(patch_ids.device) # 256 
                    x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1]) # B,256,128->B*256,128  #对k采样
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))] #对k采样完成了，现在patch_id只用于return，因此可以直接采样q的
            else:
                x_sample = feat_reshape
                patch_id = []
            if self.use_mlp:
